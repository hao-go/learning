...
###9.设置webpack的自动打包
    ...
    实现自动打包功能的步骤如下：
        ...
        B.修改package.json中的dev指令如下：
            "scripts":{
                "dev":"webpack-dev-server"//需要改成，webpack server
            }
...
在自动打包完毕之后，默认打开服务器网页，实现方式就是打开package.json文件，修改dev命令：
    "dev": "webpack-dev-server --open --host 127.0.0.1 --port 9999"//依次是自动打开、IP地址、端口号
        ...
        module : {
            rules:[
                ...
                },
                {
                    ...
                    test:/\.jpg|png|gif|bmp|ttf|eot|svg|woff|woff2$/,
                    //limit用来设置字节数，只有小于limit值的图片，才会转换
                    //为base64图片，这种格式的图片便于加载，高于或等于限制的字节数都是原来的图片格式，不会被转化
                    use:"url-loader?limit=16940"
                },{
                    test:/\.js$/,
                    use:"babel-loader",
...
值得注意的是：在webpack项目中，在此时导入的vue是阉割版的vue ，只支持render函数来导入渲染
